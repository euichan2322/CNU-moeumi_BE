...
if response.status_code == 200:
    ...
    ul = soup.select_one('#kboard-default-latest > table > tbody')
    titles = ul.select('tr > td.kboard-latest-title > a > div')
    links = ul.select('tr > td.kboard-latest-title')#kboard-default-latest > table > tbody > tr:nth-child(1) > td.kboard-latest-date
    ...
    for i in range(len(titles)):
        ...
        if respones2.status_code == 200:
            ...
            timestamp.append(soup2.select_one('#kboard-default-document > div.kboard-document-wrap > div.kboard-detail > div.detail-attr.detail-date > div.detail-value').text)
        else :
            print(respones2.status_code)
    mapped_list = [{"title": titles[i], "link": links[i], "timestamp" : timestamp[i]} for i in range(len(titles))]

    for item in mapped_list:
        item['business_group_id'] = 4
    print(mapped_list)
